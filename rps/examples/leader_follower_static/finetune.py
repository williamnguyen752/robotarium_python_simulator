...
def resilient_switching_controller(x, L, attack_type, weights, leader_idx, follower_idx, waypoints, state):
    ...
    for i in follower_idx:
        if attack_type[i] == 0:  # No attack
            ...
            # Use a weighted average of the neighbors' states
            neighbors = np.nonzero(L[i])[0]
            neighbor_weights = weights[neighbors]
            neighbor_weights /= np.sum(neighbor_weights)  # Normalize the weights
            u[:, i:i+1] = np.dot(x[:, neighbors], neighbor_weights).reshape((2, 1))
        elif attack_type[i] == 2:  # DoS attack
            # Use the leader's velocity as the reference velocity for the follower
            leader_velocity = leader_controller(x[:, leader_idx].reshape((2, 1)), waypoints[:, state].reshape((2, 1)))
            u[:, i:i+1] = leader_velocity
...
def kalman_filter_consensus(x, L, Q, R, attack_type, weights, leader_idx, follower_idx, waypoints, state):
    ...
    # Residual dynamics
    y = C.dot(x)  # Define y as the actual output
    residual = y - y_hat
    
    # Estimation error dynamics
    estimation_error = x - x_hat
    
    # Formation output prediction
    desired_formation = np.array([[0, 1, 2, 3], [0, 0, 0, 0]])  # Example desired formation
    formation_output_prediction = C.dot(desired_formation)
    ...
    for i in range(m):
        ...
        if len(neighbors) > 0:
            ...
            y = C.dot(x[:, i])  # Actual output for the i-th robot
            S = C.dot(P).dot(C.T) + R
            K = P.dot(C.T).dot(np.linalg.inv(S))
            x_hat[:, i] += K.dot(y - C.dot(x_hat[:, i]))  # Update the estimate using the residual
    ...
    print("Formation output prediction:", formation_output_prediction)
    
    return dxi, x_hat, residual, estimation_error, formation_output_prediction
    # return u.reshape((n, m)), x_hat
...
for t in range(iterations):
    ...
    # dxi = kalman_filter_consensus(xi, L, Q, R)
    # dxi, x_hat = kalman_filter_consensus(xi, L, Q, R, attack_type, weights, leader_index, follower_indices, waypoints, state)
    # Integrate the Kalman filter-based consensus controller
    dxi, x_hat, residual, estimation_error, formation_output_prediction = kalman_filter_consensus(xi, L, Q, R, attack_type, weights, leader_index, follower_indices, waypoints, state)
    # Print important parameters
    print("Iteration:", t)
    print(f"Attack type: {attack_type}")
    print(f"Leader index: {leader_index}")
    print(f"Follower indices: {follower_indices}")
    print("Residual norm:", np.linalg.norm(residual))
    print("Estimation error norm:", np.linalg.norm(estimation_error))
    # Formation output prediction
    desired_formation = np.array([[0, 1, 2, 3], [0, 0, 0, 0]])  # Example desired formation
    print("Formation output prediction error:", np.linalg.norm(formation_output_prediction - desired_formation))
    
    # Apply extended barrier certificates
    # dxi = extended_barrier_certificates(dxi, xi, safety_radius)

    # Check if the leader is compromised
    if attack_type[leader_index] != 0:
        # If the leader is compromised, assign a new leader
        healthy_robots = np.where(attack_type == 0)[0]
        if len(healthy_robots) > 0:
            leader_index = healthy_robots[0]
            follower_indices = np.setdiff1d(np.arange(N), leader_index)
        else:
            # If all robots are compromised, stop the movement
            dxi = np.zeros_like(dxi)
    else:
        # Leader
        # Apply the resilient switching controller
        leader_velocity = leader_controller(x[:2, [leader_index]], waypoints[:, state].reshape((2, 1)))
        dxi[:, [leader_index]] = leader_velocity
        
        if np.linalg.norm(x[:2, [leader_index]] - waypoints[:, state].reshape((2, 1))) < close_enough:
            state = (state + 1) % 4
        
        follower_velocities = resilient_switching_controller(xi, L, attack_type, weights, leader_index, follower_indices, waypoints, state)
        dxi[:, follower_indices] = follower_velocities[:, follower_indices]

    # Apply the resilient switching controller
    dxi = resilient_switching_controller(xi, L, attack_type, weights, leader_index, follower_indices, waypoints, state)
